...
class User(db.Model):
    ...
    def import_data(self, data):
        try:
            self.username = data['username'] if data['username'] else self.username
            self.pic = data['pic'] if data['pic'] else self.pic
            self.describe = data['describe'] if data['describe'] else self.describe
        except KeyError as e:
            raise ValidationError('Invalid class: missing ' + e.args[0])
        return self
